...
.DEFAULT_GOAL := help
.PHONY: sayhi greet blog calculator help
project := sayhi greet calculator blog

all: $(project) ## Generate Pbs and build

sayhi: $@ ## Generate Pbs and build for sayhi
...
clean: clean_sayhi clean_greet clean_calculator clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_sayhi: ## Clean generated files for greet
	${RM_F_CMD} sayhi/${PROTO_DIR}/*.pb.go
